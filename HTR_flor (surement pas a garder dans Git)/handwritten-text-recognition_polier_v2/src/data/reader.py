...
class Dataset():
    ...
    def _bentham(self):
        ...
        #print(len(paths['train']), len(paths['valid']), len(paths['test']))
        ...
        print('Remember to modify reader.py if creating hdf5 for inference!')
        for i in self.partitions:
            for line in paths[i]:
                dataset[i]['dt'].append(os.path.join(img_path, f"{line}.png"))
                #dataset[i]['gt'].append(gt_dict[line]) # MODIFY HERE, set to any number (cannot be empty)
                dataset[i]['gt'].append('sans transcription') # REPLACE WITH THIS LINE
        ...
        for i in self.partitions:
            for line in paths[i]:
                try:
                    ...
                    dataset[i]['gt'].append('sans transcription')
